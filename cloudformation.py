...
class CloudFormation(object):
    ...
    def __init__(self, profile=None, region=None,
        ...
        self.client_glue = session.client('glue', **client_kwargs)
    ...
    def validate_template(self, template_name, load_template=None, bypass=None):
        """
    ...
    def resource_policy(self, policy):
        """
        Dict with Version 2012-10-17,
        Statement[{Sid, Effect, Action[], Principal{aws}, Resource[]}]
        :param policy: dict() of policy
        :return:
        """
        policy_str = json.dumps(policy)
        try:
            response = self.client_glue.put_resource_policy(
                PolicyInJson=policy_str,
                EnableHybrid='TRUE'
            )
            return response
    ...
    def get_resource_policy(self, resource_arn):
        """
        :param resource_arn: str() of arn ex:
            arn:aws:glue:{REGION}:{PRODUCER_ACCOUNT_ID}:catalog
        :return:
        """
        try:
            response = self.client_glue.get_resource_policy(
                ResourceArn=resource_arn)
            return response
        except Exception as error:
            print(error)

    def bucket_policy_update(self, bucket_name, bucket_policy):
        """
        Dict with Version 2012-10-17,
        Statement[{Sid, Effect, Action[], Principal{aws}, Resource[]}]
        :param bucket_policy: dict() of policy
        :param bucket_name: str() of bucket name
        :return:
        """
        policy_str = json.dumps(bucket_policy)
        try:
            response = self.client_s3.put_bucket_policy(Bucket=bucket_name, Policy=policy_str)
            return response
        except Exception as error:
            print(error)
